...
- 00_refprep.md describes how I prepared my reference genomes.
- 01_trimming-mapping.sh is a script to process all raw genomes.
- 02_haplotypecalling.sh is a script to call, merge, genotype, and filter reliable variants.
...
- 09_smcpp.sh is a script to run SMC++ on all samples, with bootstrapping.
    ...
    - smcpp_makecsv.py is a custom python script used for exporting results to a csv.
    - smcpp_plot.R is a custom R script used to plot SMC++ results.
- 10_roh.sh is a script to run PLINK to calculate runs of homozygosity for all samples.
    - roh_plot.R is a custom R script used to plot ROH results.
