...
[01_trimming-mapping.sh](./01_trimming-mapping.sh) is a script to process all raw genomes 🗺️

[02_haplotypecalling.sh](./02_haplotypecalling.sh) is a script to call, merge, genotype, and filter reliable variants 🔡

03_bqsr.sh is a script to perform BQSR on all samples ⚖️
...
06_pca.sh is a script to run PCA on all samples, with options to subsample different datasets 📍

07_admixture.sh is a script to run ADMIXTURE on all samples, with options to subsample different datasets 🗂️

08_psmc.sh is a script to run PSMC on all samples 🕰️

09_smcpp.sh is a script to run SMC++ on all samples, with bootstrapping　🕰️
